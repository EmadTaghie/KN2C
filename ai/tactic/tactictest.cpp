...
//    qDebug() <<"varede tactic1 mishavad";
    if(wm->ourRobot[6].Status == AgentStatus::FollowingBall)
    {target = Vector2D(-2500,0);
//    qDebug() <<"varede follow mishavad";
    }
    else if(wm->ourRobot[6].Status == Kicking){
            ...
            if(predictRobatLocation.x > Field::MinX && predictRobatLocation.x < 0){
                ...
                case 0:
                    ...
                    if((intersect1 - predictRobatLocation).length() > (intersect2 - predictRobatLocation).length())
                        ...
                        target = intersect2;
                    if((wm->ourRobot[6].pos.loc - target).length() < 50)
                        state = 1;
                    break;
                case 1:
                    rc.useNav = false;
                    ...
                    if((intersect1 - predictRobatLocation).length() > (intersect2 - predictRobatLocation).length())
                        ...
                        target = intersect2;
                    rc.fin_pos.dir = (predictRobatLocation - wm->ourRobot[6].pos.loc).dir().radian();
                ...
                case 2:
                    rc.fin_pos.dir = (predictRobatLocation - wm->ourRobot[6].pos.loc).dir().radian();
                    rc.kickspeedx = 5;
                    if((wm->ball.pos.loc - wm->ourRobot[6].pos.loc).length() > 150 || !wm->ball.isValid){
                        rc.kickspeedx = 0;
                        state = 0;
                    }
                    break;
    ...
    else if(wm->ourRobot[6].Status == RecievingPass){
...
//    qDebug() << " target  x= " <<target.x<<" y= "<<target.y;
    ...
    rc.fin_pos.dir = (wm->ball.pos.loc - wm->ourRobot[6].pos.loc).dir().radian();
    rc.maxSpeed = 4;
    //************************************************
